...
/**********************************************************/
/*  function get_code_table2 and its supporting functions */
/*  (This function reads in a table of codesets.)         */
/**********************************************************/

  /************************************************************/
  /*                                                          */
  /*  gct_analyze_line2(pline,ps1,use_add_file,perr)          */
  /*                                                          */
  /*  Analyzes an input line for function Get_codes.          */
  /*  Input lines have 4 fields: file name (without type),    */
  /*  type, nmax, and order.                                  */
  /*  When calling the function to get a pcodeset, iff        */
  /*  use_add_file == T, adds in the contents of an           */
  /*  ADD file of additional entries if there is one.         */
  ...
  /************************************************************/
  void gct_analyze_line2(char *pline, TABLE_OF_CODESETS *ps1, int use_add_file, int *perr)
{
  char *proc = "gct_analyze_line2";
  char *pix;
...
/* get pcode from file pfname */
  ...
  get_pcode4(ps1->cd[icode].pc,
             ps1->directory,
             ps1->cd[icode].pfname,
             use_add_file,
             perr2);
  if (*perr2 > 0)
    {fprintf(stderr,"%s: get_pcode4 returns err=%d\n",proc,*perr2);
...
EXIT:
  ...
  /************************************************************/
  /*                                                          */
  /*  get_code_table2(ps1,fname,use_add_file,perr)            */
  /*                                                          */
  /*  Reads in a structure consisting of several pcodes,      */
  /* from an ASCII text file, and encode the lower codes.     */
  /*  Iff use_add_file == T, adds in the contents of ADDS     */
  /* files of additional entries if there are any.            */
  ...
  /*          14 iff pcode has more elements than the nmax    */
  /*                 that was declared in the table.          */
  /*          15 iff fatal error from get_pcode4.             */
  ...
  /*                                                          */
  /************************************************************/
  void get_code_table2(TABLE_OF_CODESETS *ps1, char *fname, int use_add_file, int *perr)
{
  /* data */
    char *proc = "get_code_table2";
    int i, dbi, err2;
    FILE *fp;
    ...
    while (fgets(pline,LINE_LENGTH,fp) != NULL) /* get next line */
      {pline = prtrim2(pline); /* trim off trailing whitespace */
       if ( (textlen(pline) > 0) && (line[0] != ';') )
         {err2 = 0;
          gct_analyze_line2(pline,ps1,use_add_file,&err2);
          if (err2 > 0)
