...
#include <phone/stdcenvf.h> /* std compile environment for functions */
  ...
  static enum edits pick_min(double dx[])
/*********************************************************/
...
db_leave_msg(proc,1); /* debug only */
 ...
 /**********************************************************************/
 /*                                                                    */
 /*  double aldistt1(pt1,ref_segs,hyp_segs,return_alignment,pt2,perr)  */
 /*                                                                    */
 /*   ALIGNMENT_INT *pt1, *pt2; int (*w)(int*,int*,pcodeset*);         */
 /*   boolean return_alignment; int *perr;                             */
 /*                                                                    */
 /* SPECIAL VERSION #1, USING TIME MARKINGS                            */
 /* DISTANCE IS THE SUM OF UNMATCHED REF AND HYP TIME                  */
 ...
 /**********************************************************************/
  ...
  double aldistt1(ALIGNMENT_INT *pt1, WTOKE_STR1 *ref_segs,
    WTOKE_STR1 *hyp_segs, int return_alignment, ALIGNMENT_INT *pt2,
    int *perr)
  {char *proc = "aldistt1";
/* data: */
   ...
   double d[MAX_SYMBS_IN_STR][MAX_SYMBS_IN_STR], distance, dx[4];
   double t2_ref, t2_hyp, dij;
...
if (db_level > 1) printf("%scalculating body cells.\n",pdb);
   for (j = 1; j <= nx; j++)
     {for (i = 1; i <= mx; i++)
        {dx[DEL] = d[i-1][j]   + ref_segs->word[i].dur; /* DEL a[i] */
         t2_ref = ref_segs->word[i].t1 + ref_segs->word[i].dur;
         t2_hyp = hyp_segs->word[j].t1 + hyp_segs->word[j].dur;
         dij = fabs(ref_segs->word[i].t1 - hyp_segs->word[j].t1)
             + fabs(t2_ref - t2_hyp);
         dx[SUB] = d[i-1][j-1] + dij; /* SUB */
         dx[INS] = d[i][j-1]   + hyp_segs->word[j].dur; /* INS b[j] */
         choice = pick_min(dx);
         d[i][j] = dx[choice];
...
/**** de-bug ****/
 if (db_level > 2) printf("%sleaving aldistt1, distance=%f\n",pdb,distance);
 db_leave_msg(proc,1); /* debug only */
/**** de-bug ****/

   return distance;
  } /* end of function "aldistt1" */
 /* end of file "aldistt1.c" */
