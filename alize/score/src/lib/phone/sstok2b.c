...
#include <phone/stdcenvf.h> /* std compile environment for functions */
 ...
 /**********************************************************************/
 /*                                                                    */
 /*   SUBSTRING sstok2b(sx,delimiters);                                */
 /*   char *sx, *delimiters;                                           */
 /*                                                                    */
 ...
 /* ALSO, in this version, the final end-of-string character '/0' acts */
 /* like a delimiter.                                                  */
 /*   If delimiter aren't found, returns a substring of length < 0     */
 /**********************************************************************/

 SUBSTRING sstok2b(char *sx, char *delimiters)
{char *proc = "sstok2b";
...
/*   move thru other contiguous delimiter characters, if any */
  while ((strchr(delimiters,*ssy->start) != NULL)&&
         (*ssy->start != '\0'))   ssy->start += 1;
  if (*ssy->start == '\0') {ssy->end = ssy->start - 2; goto RETURN;}
