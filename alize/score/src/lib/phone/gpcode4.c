...
#include <phone/stdcenvf.h> /* std compile environment for functions */
  ...
  /************************************************************/
  /*                                                          */
  /*  get_pcode4(p1,path,fname,use_add_file,perr)             */
  /*                                                          */
  /*  Function #4 to read in a pcodeset from a file.          */
  /*  Input: *path+*fname contains the name of the file       */
  /*         Iff use_add_file == T, adds in the contents of an*/
  ...
  /*          13 iff there was a memory ovf.                  */
  /*          14 iff get_ppfcn2 returns an error indication.  */
  /* (Uses global parameter LONG_LINE.)                       */
  /*                                                          */
  /* Based on the gpcode3.c of 7/20/94, adding the parameter  */
  /* "use_add_file".                                          */
  /************************************************************/
  void get_pcode4(pcodeset *p1, char *path, char *fname, int use_add_file, int *perr)
{
/* data */
    FILE *fp;
...
/* issue warning if an *.ADD file exists but isn't to be used */
    if (!use_add_file) warn_if_add_file(full_fname,"start");
/* initialize  */
  ...
  /* add contents of ADD file if so requested */
    if (use_add_file) add_add_file(p1,full_fname,perr);
    if (*perr > 0)
      {fprintf(stderr,"*ERR:%s: add_add_file err %d\n",proc,*perr);
       goto RETURN;
      }
