...
void calc_mean_var_std_dev_Zstat(int *Z_list, int num_Z, float *mean, float *variance, float *std_dev, float *Z_stat)
{
    int i;
    float tmp = 0.0;
...
/**********************************************************************/
/*  given a list of floats, calculate the mean, variance,             */
/*  standard deviation and Z_statistic                                */
void calc_mean_var_std_dev_Zstat_float(float *Z_list, int num_Z, float *mean, float *variance, float *std_dev, float *Z_stat)
{
    int i;
    float tmp = 0.0;
...
int print_Z_analysis(float Z_stat)
{
    int i;
...
/**********************************************************************/
/* calc_two_sample_z_test_float by Brett 5/10/93                      */ 
/*     a general routine to perform a two-sample z test on two lists  */
/*     of floats                                                      */
/**********************************************************************/
void calc_two_sample_z_test_float(float *l1, float *l2, int num_l1, int num_l2, float *Z)
{
  float mean_l1,var_l1,sd_l1,Z_stat_l1;
  float mean_l2,var_l2,sd_l2,Z_stat_l2;
  calc_mean_var_std_dev_Zstat_float(l1,num_l1,&mean_l1,&var_l1,&sd_l1,&Z_stat_l1);
  calc_mean_var_std_dev_Zstat_float(l2,num_l2,&mean_l2,&var_l2,&sd_l2,&Z_stat_l2);
  *Z=((mean_l1-mean_l2)/sqrt(((sd_l1*sd_l1)/(float)num_l1)+((sd_l2*sd_l2)/(float)num_l2)));
  /* print_Z_analysis(*Z);*/
}
...
double compute_acc_binomial(int R, int n, double p)
{
    ...
    /* printf("Binomial 7,5,0.5 = %f\n",(float)compute_acc_binomial(1,1,0.1));*/
