...
void waveform_update(char *, char *, char *, int);
...
void print_usage(void) {
    fprintf(spfp,
  "Usage: tsphere -[vmMehw] [-l iterations] [ -[bf] n ]  -d sphere_lib_dir\n");
}

int warning = 0;
int quit_on_warning = 0;
...
int main(int argc, char **argv)
{
    ...
    while (( c=hs_getopt( argc, argv, "d:l:b:f:hvmMew" )) != -1 )
	switch ( c ) {
	  case 'd':
...
int verify_checksum_computations(int test){
	...
	fprintf(spfp,"Proc %s: Long int is only %ld bytes, >2 needed\n",
		proc,(long int) sizeof(long));
    if (sizeof(short) != 2)
	fprintf(spfp,"Proc %s: short int is only %ld bytes, != 2\n",
		proc,(long int) sizeof(short));
...
void selective_channel_test(int test){
    ...
    waveform_update(EX5_2CHAN,"SE-PCM:SBF-10:CH-1+2",EX5_CHAN12_PCM, 1);

    fprintf(spfp,"---- Write channels added together\n");
    waveform_update(EX5_2CHAN,"CH-1+2", EX5_CHAN12, 1);
    waveform_update(EX5_2CHAN_PCM,"SE-PCM:SBF-10:CH-1+2", EX5_CHAN12_PCM, 1);

    fprintf(spfp,"---- Write selective channels changing sample encodings\n");
    waveform_update(EX5_2CHAN,"SE-PCM:SBF-10:CH-1", EX5_CHAN1_PCM, 1);
    waveform_update(EX5_2CHAN,"SE-PCM:SBF-10:CH-2", EX5_CHAN2_PCM, 1);

    fprintf(spfp,"---- Write selective channels\n");
    waveform_update(EX5_2CHAN_PCM,"SE-PCM:SBF-10:CH-1", EX5_CHAN1_PCM, 1);
    waveform_update(EX5_2CHAN_PCM,"SE-PCM:SBF-10:CH-2", EX5_CHAN2_PCM, 1);
    waveform_update(EX5_2CHAN,"SE-ULAW:SBF-1:CH-1",EX5_CHAN1, 1);
    waveform_update(EX5_2CHAN,"SE-ULAW:SBF-1:CH-2",EX5_CHAN2, 1);
    fprintf(spfp,"\n");
}
...
void ulaw_test(int test)
{
    ...
    waveform_update(EX1_10,"SE-ULAW:SBF-1",EX4_ULAW, 1);
    waveform_update(EX1_01,"SE-ULAW:SBF-1",EX4_ULAW, 1);

    fprintf(spfp,"---- Write as 2 Byte PCM file\n");
    waveform_update(EX4_ULAW,"SE-PCM-2:SBF-10",EX4_ULAW_10, 1);
    waveform_update(EX4_ULAW,"SE-PCM-2:SBF-01",EX4_ULAW_01, 1);
    waveform_update(EX4_ULAW,"SE-SHORTEN",EX4_ULAW_SHORTEN, 0);
    waveform_update(EX4_ULAW,"SE-WAVPACK",EX4_ULAW_WAVPACK, 1);
    waveform_update(EX4_ULAW_WAVPACK,"SE-ULAW",EX4_ULAW, 1);
    waveform_update(EX4_ULAW_SHORTEN,"SE-ULAW",EX4_ULAW, 1);
    waveform_update(EX4_ULAW_WAVPACK,"SE-PCM-2:SBF-10",EX4_ULAW_10, 1);
    waveform_update(EX4_ULAW_SHORTEN,"SE-PCM-2:SBF-01",EX4_ULAW_01, 1);
...
void pculaw_test(int test)
{
    ...
    fprintf(spfp,"---- Test creation from the original\n");
    waveform_update(EX1_10,"SE-PCULAW:SBF-1",EX7_PCULAW, 1);
    waveform_update(EX1_01,"SE-PCULAW:SBF-1",EX7_PCULAW, 1);

    fprintf(spfp,"---- Write as 2 Byte PCM file\n");
    waveform_update(EX7_PCULAW,"SE-PCM-2:SBF-10",EX7_PCULAW_10, 1);
    waveform_update(EX7_PCULAW,"SE-PCM-2:SBF-01",EX7_PCULAW_01, 1);
    waveform_update(EX7_PCULAW,"SE-SHORTEN",EX7_PCULAW_SHORTEN, 0);
    waveform_update(EX7_PCULAW,"SE-WAVPACK",EX7_PCULAW_WAVPACK, 1);
    waveform_update(EX7_PCULAW_WAVPACK,"SE-PCULAW",EX7_PCULAW, 1);
    waveform_update(EX7_PCULAW_SHORTEN,"SE-PCULAW",EX7_PCULAW, 1);
    waveform_update(EX7_PCULAW_WAVPACK,"SE-PCM-2:SBF-10",EX7_PCULAW_10, 1);
    waveform_update(EX7_PCULAW_SHORTEN,"SE-PCM-2:SBF-01",EX7_PCULAW_01, 1);
				 ...
				 char *chan1_wav, char *chan2_wav)
{
  ...
  CLEAN_UP:
	...
	if ((rtn = diff_data(combined_wav,new_wav,0,spfp)) != 0){
	  if (rtn == 1000) {
	    ; /* This is not worth warning about because it is a byte-swap issue */
	  } else  {
	    diff_data(combined_wav,new_wav,1,spfp);  /* Re-run verbosely */
	    fprintf(spfp,"Warning: files '%s' and '%s' decode to the",
		    combined_wav,new_wav);
	    if (rtn == 100) 
		fprintf(spfp," same form, but are not identical on disk\n");
	    else 
		fprintf(spfp," same form, but fail another test comparing data on disk\n");
	    warning++;
            if (quit_on_warning)
              exit(-1);
	  }
...
void update_test(int test)
{
    ...
    waveform_update(EX1_10,"SBF-10",EX1_10, 1);
    waveform_update(EX1_10,"SBF-01",EX1_01, 1);
    waveform_update(EX1_10,"SE-WAVPACK:SBF-10",EX1_10_WAVPACK, 1);
    waveform_update(EX1_10,"SE-SHORTEN:SBF-10",EX1_10_SHORTEN, 0);
    waveform_update(EX1_10,"SE-WAVPACK:SBF-01",EX1_01_WAVPACK, 1);
    waveform_update(EX1_10,"SE-SHORTEN:SBF-01",EX1_01_SHORTEN, 0);

    waveform_update(EX1_01,"SBF-01",EX1_01, 1);
    waveform_update(EX1_01,"SBF-10",EX1_10, 1);
    waveform_update(EX1_01,"SE-WAVPACK:SBF-01",EX1_01_WAVPACK, 1);
    waveform_update(EX1_01,"SE-SHORTEN:SBF-01",EX1_01_SHORTEN, 0);
    waveform_update(EX1_01,"SE-WAVPACK:SBF-10",EX1_10_WAVPACK, 1);
    waveform_update(EX1_01,"SE-SHORTEN:SBF-10",EX1_10_SHORTEN, 0);

    waveform_update(EX1_10_SHORTEN,"SE-SHORTEN:SBF-10",EX1_10_SHORTEN, 0);
    waveform_update(EX1_10_SHORTEN,"SE-SHORTEN:SBF-01",EX1_01_SHORTEN, 0);
    waveform_update(EX1_10_SHORTEN,"SE-WAVPACK:SBF-10",EX1_10_WAVPACK, 1);
    waveform_update(EX1_10_SHORTEN,"SE-WAVPACK:SBF-01",EX1_01_WAVPACK, 1);
    waveform_update(EX1_10_SHORTEN,"SE-PCM:SBF-10",EX1_10, 1);
    waveform_update(EX1_10_SHORTEN,"SE-PCM:SBF-01",EX1_01, 1);

    waveform_update(EX1_10_WAVPACK,"SE-WAVPACK:SBF-10",EX1_10_WAVPACK, 1);
    waveform_update(EX1_10_WAVPACK,"SE-WAVPACK:SBF-01",EX1_01_WAVPACK, 1);
    waveform_update(EX1_10_WAVPACK,"SE-SHORTEN:SBF-10",EX1_10_SHORTEN, 0);
    waveform_update(EX1_10_WAVPACK,"SE-SHORTEN:SBF-01",EX1_01_SHORTEN, 0);
    waveform_update(EX1_10_WAVPACK,"SE-PCM:SBF-10",EX1_10, 1);
    waveform_update(EX1_10_WAVPACK,"SE-PCM:SBF-01",EX1_01, 1);


    waveform_update(EX1_10_SHORTPACK,"SE-SHORTEN:SBF-10",EX1_10_SHORTEN, 0);
    waveform_update(EX1_10_SHORTPACK,"SE-SHORTEN:SBF-01",EX1_01_SHORTEN, 0);
    waveform_update(EX1_10_SHORTPACK,"SE-WAVPACK:SBF-10",EX1_10_WAVPACK, 1);
    waveform_update(EX1_10_SHORTPACK,"SE-WAVPACK:SBF-01",EX1_01_WAVPACK, 1);
    waveform_update(EX1_10_SHORTPACK,"SE-PCM:SBF-10",EX1_10, 1);
    waveform_update(EX1_10_SHORTPACK,"SE-PCM:SBF-01",EX1_01, 1);

    waveform_update(EX1_01_SHORTEN,"SE-SHORTEN:SBF-01",EX1_01_SHORTEN, 0);
    waveform_update(EX1_01_SHORTEN,"SE-SHORTEN:SBF-10",EX1_10_SHORTEN, 0);
    waveform_update(EX1_01_SHORTEN,"SE-WAVPACK:SBF-10",EX1_10_WAVPACK, 1);
    waveform_update(EX1_01_SHORTEN,"SE-WAVPACK:SBF-01",EX1_01_WAVPACK, 1);
    waveform_update(EX1_01_SHORTEN,"SE-PCM:SBF-10",EX1_10, 1);
    waveform_update(EX1_01_SHORTEN,"SE-PCM:SBF-01",EX1_01, 1);

    waveform_update(EX1_01_WAVPACK,"SE-WAVPACK:SBF-01",EX1_01_WAVPACK, 1);
    waveform_update(EX1_01_WAVPACK,"SE-WAVPACK:SBF-10",EX1_10_WAVPACK, 1);
    waveform_update(EX1_01_WAVPACK,"SE-SHORTEN:SBF-10",EX1_10_SHORTEN, 0);
    waveform_update(EX1_01_WAVPACK,"SE-SHORTEN:SBF-01",EX1_01_SHORTEN, 0);
    waveform_update(EX1_01_WAVPACK,"SE-PCM:SBF-10",EX1_10, 1);
    waveform_update(EX1_01_WAVPACK,"SE-PCM:SBF-01",EX1_01, 1);

    waveform_update(EX1_01_SHORTPACK,"SE-SHORTEN:SBF-01",EX1_01_SHORTEN, 0);
    waveform_update(EX1_01_SHORTPACK,"SE-SHORTEN:SBF-10",EX1_10_SHORTEN, 0);
    waveform_update(EX1_01_SHORTPACK,"SE-WAVPACK:SBF-10",EX1_10_WAVPACK, 1);
    waveform_update(EX1_01_SHORTPACK,"SE-WAVPACK:SBF-01",EX1_01_WAVPACK, 1);
    waveform_update(EX1_01_SHORTPACK,"SE-PCM:SBF-10",EX1_10, 1);
    waveform_update(EX1_01_SHORTPACK,"SE-PCM:SBF-01",EX1_01, 1);
...
void waveform_update(char *base_file, char *sdm_mode, char *compare_file, int on_disk_compare_required)
{
    SP_FILE *sp;
    int rtn;
